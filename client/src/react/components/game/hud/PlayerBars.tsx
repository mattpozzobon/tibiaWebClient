...
const Bar: React.FC<BarProps> = ({ current, max, color, lowColor, icon }) => {
  ...
  return (
    <div className="player-bar">
      ...
      <div className="bar-container">
        ...
        />
        <div className="bar-value-overlay">{current} / {max}</div>
      </div>
    </div>
  );
};
...
const PlayerBars: React.FC<PlayerBarsProps> = ({ gameClient }) => {
  ...
  return (
    <div id="player-bars" className="player-bars-container">
      ...
      <Bar current={vitalValues.capacity} max={vitalValues.maxCapacity} color="#9E9E9E" lowColor="#FFC107" icon="🎒" />
    </div>
  );
};
