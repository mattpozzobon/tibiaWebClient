...
export default class ItemRenderer {
  ...
  public collectSpritesForTile(tile: Tile, screenPos: Position, spriteBatches: Map<string, BatchSprite[]>): void {
    ...
    for (let i = 0; i < items.length; ++i) {
      ...
      this.collectSpriteForItem(item, screenPos, tile.__renderElevation, 32, spriteBatches, outlineThis);
  ...
  public collectOnTopSpritesForTile(tile: Tile, screenPos: Position, spriteBatches: Map<string, BatchSprite[]>): void {
    const items: Item[] = tile.items;
    const currentHoverTile = window.gameClient.mouse.getCurrentTileHover();
    ...
    for (let i = 0; i < items.length; ++i) {
      ...
      this.collectSpriteForItem(item, screenPos, 0, 32, spriteBatches, outlineThis);
    }
  }
